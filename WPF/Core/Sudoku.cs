...
namespace GeniusSudoku.Core
{
    ...
    public class Sudoku
    {
        ...
        public void Save(XmlDocument aDoc, string ID, params string[] Attrs)
        {
            XmlNode n = null;
            n = Find(aDoc, ID);
            if (n != null)
                aDoc.DocumentElement.RemoveChild(n);
            XmlElement elem = aDoc.CreateElement("Partie");
            Attributes(elem, "ID", ID);
            Attributes(elem, "Date", DateTime.Now.ToString());
            Attributes(elem, "Fixed", FixedNumbers());
            Attributes(elem, "User", UserNumbers());
            Attributes(elem, "Rate", _Rate.ToString());
            Attributes(elem, "Notes", Notes());
            Attributes(elem, "Flags", Flags());
            if (Attrs != null)
            {
                for (int i = 0; i < Attrs.Length; i+=2)
                    Attributes(elem, Attrs[i], Attrs[i+1]);                    
            }
        ...
        private XmlAttribute Attributes(XmlElement elem, string attrName, string attrValue)
        {
            XmlAttribute attr = elem.OwnerDocument.CreateAttribute(attrName);
        ...
        private XmlNode Find(XmlDocument doc, string ID)
        {
            if (doc.DocumentElement != null)
                return doc.DocumentElement.SelectSingleNode("./Partie[@ID='" + ID + "']");
            return null;
        }
        ...
        public void Restore(XmlDocument aDoc, string ID)
        {
            XmlNode n = Find(aDoc, ID);
            if (n != null)
            {
                _IDPartie = n.Attributes["ID"].Value;
                SetTableau(n.Attributes["Fixed"].Value, n.Attributes["User"].Value);
                if (n.Attributes["Notes"] != null)
                    RestoreNotes(n.Attributes["Notes"].Value);

                if (n.Attributes["Flags"] != null)
                    RestoreFlags(n.Attributes["Flags"].Value);

                if (n.Attributes["Rate"] != null)
                    _Rate = int.Parse(n.Attributes["Rate"].Value);
            }
        }
